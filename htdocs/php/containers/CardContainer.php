...
class CardContainer extends HTMLContent
{
    ...
    public function onCreateAdminHtml() : string
    {
        ...
        $this->appendHtml(<<<HTML
            ...
            <div class="element-title">Contenu des cartes</div>
        ...
        {
            $this->appendHtml(<<<HTML
                <button role="admin" target="image" container-id="{$this->id}" id="{$value->id}" value="{$value->getImage()}" placeholder="URL Image">{$value->getImage()}</button>
                <input role="admin" target="text" container-id="{$this->id}" id="{$value->id}" value="{$value->getText()}" placeholder="Texte">
            HTML);
        }
