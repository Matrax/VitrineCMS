...
class Paragraph extends HTMLContent
{
    ...
    public function onCreateHtml() : string
    {
        $this->appendHtml(<<<HTML
            <div class="paragraph">
            <img class="paragraph-image" src="{$this->image}" alt="image" width=100% height=100%>
            <div class="paragraph-container">
            <div class="paragraph-title">{$this->title}</div>
            <div class="paragraph-text">{$this->text}</div>
            </div>
            </div>
        HTML);
    ...
    public function onCreateAdminHtml() : string
    {
        $this->appendHtml(<<<HTML
            ...
            <button role="admin" target="image" id="{$this->id}" value="{$this->image}" placeholder="{URL Image}">{$this->image}</button>
